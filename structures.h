...
typedef struct {
    ...
    int posX;            // Posicion X de esta pantalla en el mundo
    int posY;            // Posicion Y de esta pantalla en el mundo
} Mapa;

// Estructura del jugador (Salchichin)
struct player
{
    int hp;       // Puntos de vida
    float xp;     // Experiencia
    int dmg;      // Dano del ataque
...
// ─────────────────────────────────────────────
// NUEVAS ESTRUCTURAS — Sistema de combate, inventario y progresion
// ─────────────────────────────────────────────
...
typedef struct {
    Item items[10];   // Maximo 10 items
    int cantidad;     // Cuantos items tiene actualmente
} Inventario;

// Skills desbloqueables por nivel
typedef struct {
    char nombre[30];  // Nombre de la skill
    char desc[60];    // Descripcion
    int nivelReq;     // Nivel requerido para desbloquear
    int dmgExtra;     // Dano adicional que aplica
    int costoHP;      // Costo en HP propio (0 si no cuesta nada)
} Skill;

// Estado completo del jugador en combate y progresion
typedef struct {
    int hp;           // HP actual
    int hpMax;        // HP maximo
    ...
    Skill skills[5];  // Skills disponibles (maximo 5)
    int numSkills;    // Cuantas skills tiene desbloqueadas
} Jugador;
